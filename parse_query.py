...
# def decomposableDisjunction
# def decomposableConjunction
...
def propogation(UCQ, quantifier, sep, list_of_separator_variables):
   ...
   common_table = find_Common_Table(
       list_of_separator_variables)  # tables whose probabilities can be substituted directly
   remove = []
   ...
   for i in range(0, len(
           remove)):  # remove common table names from each conjunctive query
       for j in remove[i]:
           del UCQ[i][j]
           if (len(UCQ[i]) == 0):
               UCQ.remove(UCQ[i])
               i -= 1
   del quantifier[
       sep]  # delete the separator variable from quantifier dictionary
   return


def find_Common_Table(
       list_of_separator_variables):  # find tables whose probabilities can be substituted directly
...
def find_Separator(UCQ, quantifier):  # find separator for entire UCQ
   ...
   for q in quantifier:
       if (quantifier[
           q] == 0):  # if variable was already used as separator, don't check for it
           continue
       quant_count = 0
       for cq in UCQ:
           for clause in cq:
...
def parse_UCQ(input_query):
   ...
   for cq in UCQ_list:  # iterate through the above list, cq is each conjunction
       ...
       for q in list_cq:  # iterate through the relations within each conjunctive clause
           ...
           temp_dict["name"] = q[0]
           temp_dict["variables"] = q[1].strip("").replace(")", "").split(
               ",")  # remove spaces, remove the ) at the endd, and split by comma to get a list of variables
           temp_tables.add(q[0])
           for var in temp_dict[
               "variables"]:  # set the quantifier value as existential for all variables
               quantifier[var] = 1
           dict_cq[
               clause_count] = temp_dict  # append relation dictionary to the conjunctive clause dictionary
           clause_count += 1
       UCQ.append(
...
# probabilities = {'S': [0.4, 0.6, 0.8]}  # dummy probabilities
...
def getProbability(Query):
   return 0.5 #substitue values

#Applying lifted inference rules
...
def probability(UCQ, quantifiers, tables):
    print("in probability function")
   # Base of recursion
    if type(UCQ) is dict and test(UCQ):
       #print("Probablity ",UCQ)
       return getProbability(UCQ)

   # decomposable disjunction
    elif isUCQ(UCQ):
        if check_Independence_UCQ(tables):
            print("ISUCQ")
           print("Cal val1",UCQ[0])
           val1 = 1 - probability(UCQ[0], quantifier, tables)
           print(val1)
           print("cal val2 for",UCQ[1])
